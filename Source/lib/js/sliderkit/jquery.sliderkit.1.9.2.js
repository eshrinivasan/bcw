	...
	SliderKit = function() {
		...
		this._autoScrollHoverStop = function(){
			...
			if(this.options.autostill){
				$( this.domObj).hover(
					...
					function(){
						//self.playBtn.removeClass(self.cssNames.btnPause);
					}
				);
			}
		...
		this.autoScrollStart = function() {
			...
			/*(function(){
			  function next(){
			    self._change(null, "-=", null, self.lineScrollDo, null);
			    var duration = self.options.autospeed;
			    if(self.currId ==0 && self.prevId ==self.allItems-1)
			      	duration = self.options.pauseonfirst;
			    	self.isPlaying =  setTimeout(next, duration);
			  }
			 self.isPlaying =  setTimeout(next, self.options.autospeed);
			})();*/

			/*(self.currId ==0 && self.prevId ==self.allItems-1) ? */
		...
		this.autoScrollPauseonFirst = function(){
			var self = this;				
				this.isPlaying = setInterval(function(){
					self._change(null, "-=", null, self.lineScrollDo, null);
				}, self.options.pauseonfirst);	
